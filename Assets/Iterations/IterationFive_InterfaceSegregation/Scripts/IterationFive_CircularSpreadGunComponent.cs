...
namespace SolidPrinciple.IterationFive
{
    public class IterationFive_CircularSpreadGunComponent : IterationFive_MachineGunComponent
    {
        ...
        public override void ShootBullet()
        {
            ...
            {
                if (Instantiate(BulletPrefab, ShootingEffect.transform.position, Quaternion.Euler(0, anglePerBullet * i, 0), ShootingEffect.transform).TryGetComponent<IterationFive_BaseBullet>(out var bullet))
                {
                    bullet.ShootBullet();
                }
            }
