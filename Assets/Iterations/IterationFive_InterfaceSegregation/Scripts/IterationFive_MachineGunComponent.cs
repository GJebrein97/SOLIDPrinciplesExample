...
namespace SolidPrinciple.IterationFive
{
    ...
    public class IterationFive_MachineGunComponent : IterationFive_BaseWeapon, IterationFive_IAttackSpeed, IterationFive_IShootable, IterationFive_IReloadable 
    {
        ...
        public virtual void ShootBullet()
        {
            if (BulletPrefab)
            {
                if (Instantiate(BulletPrefab, ShootingEffect.transform).TryGetComponent<IterationFive_BaseBullet>(out var bullet))
                {
                    bullet.ShootBullet();
                }
            }
